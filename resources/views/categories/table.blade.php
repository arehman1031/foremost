<div class="table-responsive">
    <table class="table" id="categories-table">
        ...
        @foreach($categories as $category)
            <tr>
                <td>{!! $category->name !!}</td>
            	<td>{!! \App\Category::$names[$category->type] !!}</td>
				<td>{!! $category->desc !!}</td>
            	<td>@if($category->is_popular) Yes @else No @endif</td>
            	<td><img src="{!! Storage::url($category->image) !!}" alt="{!! $category->name !!}" height="100px" width="200px"></td>
                <td>
